...
# File descriptor for the lock (use high number to avoid conflicts)
readonly _NFTBAN_NFT_LOCK_FD=200
...
nftban_nft_write_lock() {
    ...
    if ! flock -x -w "$timeout" "$_NFTBAN_NFT_LOCK_FD" 2>/dev/null; then
        log_warn "nft write lock timeout after ${timeout}s"
        return 1
    fi
    return 0
}
...
nftban_nft_read_lock() {
    local timeout="${1:-5}"
    exec 200>"$NFTBAN_NFT_LOCK"
    if ! flock -s -w "$timeout" "$_NFTBAN_NFT_LOCK_FD" 2>/dev/null; then
        log_warn "nft read lock timeout after ${timeout}s"
        return 1
    fi
    return 0
}

# nftban_nft_unlock — release the lock
nftban_nft_unlock() {
    flock -u "$_NFTBAN_NFT_LOCK_FD" 2>/dev/null
}
