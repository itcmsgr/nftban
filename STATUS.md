...
**Version (this commit):** v1.112.0 — sourced from [`/VERSION`](VERSION); static per commit, not auto-updated. For the current released tag see [GitHub releases](https://github.com/itcmsgr/nftban/releases) or the README badge.
...
**Schema status:** schema remains frozen at `1.83.0` in v1.112.0. **No new metric names beyond those already declared in `17_SCHEMA_AND_CONTRACT_FINAL.md` §F4 + already accepted in the receiver-v2 181-entry allow-list since v1.111.0 publish.** **No new label cardinality dimensions.** **No nft kernel set/counter entities added.** **No health JSON schema entries added.** PR #604 implements 6 host-vitals Prometheus emissions for names that have been part of the frozen schema contract since v1.90 + accepted by the consumer-side allow-list since v1.111.0; per operator contract-fulfill interpretation locked at `V112_CONTRACT_FULFILL_ATTESTATION.md`, this is schema fulfillment (implementing already-declared names), not schema mutation. The three v1.100.4 H4 disclosure items (ban attribution under `source=manual`, `EffectiveIdle` for Portscan/LoginMon, shared `input_syn_rate_exceeded` counter) are unchanged. PR-M2c new nft named counters and PR-M2d kernel set element annotation cookies remain explicit schema-unfreeze items deferred to v1.113+ (need separate `OPEN-V1XX-SCHEMA-UNFREEZE-*` gates). **No portal, install API, panel-adapter, FHS, sysusers, polkit, dns2-migration-execution, DEGRADED-runtime, eventbus, packaging, or systemd changes in v1.112.0.** v1.112.0 is a focused single-PR PR-M2b-w1 host-vitals emission fulfillment; behavior change confined to 6 already-declared host-vitals metric names now actually emitting on the daemon `/metrics` surface (the receiver-v2 has been ready since 2026-05-02 M-T9 cutover).
