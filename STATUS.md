...
**Version (this commit):** v1.111.0 — sourced from [`/VERSION`](VERSION); static per commit, not auto-updated. For the current released tag see [GitHub releases](https://github.com/itcmsgr/nftban/releases) or the README badge.
...
**Schema status:** schema remains frozen at `1.83.0` in v1.111.0. **No new metric names, labels, registrations, health JSON schema entries, or nft kernel set/counter entities** across the V111 PR-A conservative lane (PR #602 wires `MetricsExporter.RecordAction` via a new `onAction`/`SetOnAction` callback; both target metric names `nftban_watchdog_action_total` and `nftban_watchdog_last_action_timestamp_seconds` were already registered at v1.110.0 via `promauto.NewCounterVec` / `promauto.NewGaugeVec` — this release only makes their counter values actually advance in production when watchdog actions fire). The three v1.100.4 H4 disclosure items (ban attribution under `source=manual`, `EffectiveIdle` for Portscan/LoginMon, shared `input_syn_rate_exceeded` counter) are unchanged. PR-M2b-w1 host vitals registration (6 new `promauto.New*` for `nftban_host_*` metric names already declared in schema doc 17 §F4 + receiver-v2 181-entry allow-list) is deferred to v1.112 schema-fulfill gate. PR-M2c new nft named counters and PR-M2d kernel set element annotation cookies remain explicit schema-unfreeze items deferred to v1.112+. **No metrics-name additions, portal, install API, panel-adapter, FHS, sysusers, polkit, dns2-migration-execution, DEGRADED-runtime, eventbus, packaging, or systemd changes in v1.111.0.** v1.111.0 is a focused single-PR D-METR-2 emission-wiring fix; behavior change confined to two already-registered watchdog counters now actually emitting non-zero values.
