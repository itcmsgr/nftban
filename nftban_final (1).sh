...
# Logging function
# Shim: structured log event (category, actor, target, subject, msg, status, tag, extra...)
_log_event() {
  local category="$1"; shift || true
  local actor="$1"; shift || true
  ...
  local rest="$*"]
  # Reuse existing log() if available
  if type log >/dev/null 2>&1; then
    log "[$category][$status][$tag] $subject: $message $rest"
  else
    echo "[$category][$status][$tag] $subject: $message $rest" >&2
  fi
}
...
Delegation (power-user):
  --init-nft [ARGS]     Delegate to $INIT_NFTABLES_SCRIPT (e.g., --init-nft setup | validate | apply)
  --init-f2b [ARGS]     Delegate to $INIT_FAIL2BAN_SCRIPT (e.g., --init-f2b setup | validate | apply)
...
Simple passthrough subcommands:
  init nft [ARGS]        Directly run $INIT_NFTABLES_SCRIPT with ARGS (stdin/stdout/stderr passthrough)
  init f2b [ARGS]        Directly run $INIT_FAIL2BAN_SCRIPT with ARGS
  nft [ARGS]             Shortcut for 'init nft [ARGS]'
  f2b|fail2ban [ARGS]    Shortcut for 'init f2b [ARGS]'
  (Tip) export NFTBAN_DELEGATE_MODE=spawn to avoid 'exec' replacement if you need to continue in nftban afterwards.
...
delegate_init_nftables() {
  local args="$*"
  # Ensure executable bit if present
  if [ -f "$INIT_NFTABLES_SCRIPT" ] && [ ! -x "$INIT_NFTABLES_SCRIPT" ]; then
    chmod +x "$INIT_NFTABLES_SCRIPT" || true
  fi
  ...
  _log_event "delegate" "" "" "ruleset" "init-nftables $args" "start" "init-nft"
  "$INIT_NFTABLES_SCRIPT" $args
  local rc=$?
  if [ $rc -eq 0 ]; then
    _log_event "delegate" "" "" "ruleset" "init-nftables $args" "success" "init-nft"
    echo -e "${GREEN}Delegated init-nftables completed successfully.${NC}"
  else
    _log_event "delegate" "" "" "ruleset" "init-nftables $args" "fail" "init-nft" "rc=$rc"
    echo -e "${RED}Delegated init-nftables failed (rc=$rc).${NC}"
  fi
  return $rc
}


# --- Minimal passthrough runner ---
# Uses exec-mode by default so exit code and output come straight from the child script.
# Set NFTBAN_DELEGATE_MODE=spawn to run without exec.
_delegate_passthrough() {
  local script="$1"; shift || true
  if [ -z "$script" ] || [ ! -f "$script" ]; then
    echo -e "${RED}Delegate script not found:${NC} $script"
    return 127
  fi
  # ensure executable bit (no-op if already executable)
  [ -x "$script" ] || chmod +x "$script" 2>/dev/null || true
  if [ "${NFTBAN_DELEGATE_MODE:-exec}" = "exec" ]; then
    exec "$script" "$@"
  else
    "$script" "$@"
    return $?
...
delegate_init_fail2ban() {
  local args="$*"
  if [ -f "$INIT_FAIL2BAN_SCRIPT" ] && [ ! -x "$INIT_FAIL2BAN_SCRIPT" ]; then
    chmod +x "$INIT_FAIL2BAN_SCRIPT" || true
  fi
  ...
  _log_event "delegate" "" "" "fail2ban" "init-fail2ban $args" "start" "init-f2b"
  "$INIT_FAIL2BAN_SCRIPT" $args
  local rc=$?
  if [ $rc -eq 0 ]; then
    _log_event "delegate" "" "" "fail2ban" "init-fail2ban $args" "success" "init-f2b"
    echo -e "${GREEN}Delegated init-fail2ban completed successfully.${NC}"
  else
    _log_event "delegate" "" "" "fail2ban" "init-fail2ban $args" "fail" "init-f2b" "rc=$rc"
    echo -e "${RED}Delegated init-fail2ban failed (rc=$rc).${NC}"
  fi
  return $rc
}
