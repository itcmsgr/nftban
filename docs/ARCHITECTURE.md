...
**Security Fix v0.32.0:** Blacklist checks now run BEFORE port checks.
...
chain input {
  ...
  # 4. CRITICAL: Blacklist BEFORE ports (v0.32.0 fix)
...
**v0.32.0 Bug:** Port checks ran before blacklist → blacklisted IPs could access SSH!
**v0.32.0 Fix:** Blacklist checks run first → blacklisted IPs are blocked from all services.
...
**v0.32.0 (2025-11-05) - Critical Security Release**
