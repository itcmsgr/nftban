...
**Security Fix v0.32.6:** Blacklist checks now run BEFORE port checks.
...
chain input {
  ...
  # 4. CRITICAL: Blacklist BEFORE ports (v0.32.6 fix)
...
**v0.32.6 Bug:** Port checks ran before blacklist → blacklisted IPs could access SSH!
**v0.32.6 Fix:** Blacklist checks run first → blacklisted IPs are blocked from all services.
...
**v0.32.6 (2025-11-05) - Critical Security Release**
