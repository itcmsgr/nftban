...
table ip nftban {
    ...
    # 65535 with NO timeout, so state only grew. At capacity the insertion of a
    # NEW source fails, the rule does not match, and the packet falls through to
    # the unconditional drop — measured DNS outages on 4 resolvers (2026-08-04)
    # and a 95% TCP-lockout trajectory on srv3. On nft <= 1.0.9 the implicit set
    # is an INLINE METER: not listable as a set, not flushable, recoverable only
    # by ruleset recreation.
    #
    # Explicit named dynamic sets fix all three measured defects at once:
    #   BOUNDED    timeout expires idle sources; state tracks the ACTIVE
    #              population instead of every source ever seen
    #   ADDRESSABLE one runtime representation on every supported nft
    #              (proven 1.0.2 / 1.0.9 / 1.1.1); `nft flush set` works
    #   EXPLICIT   size + timeout declared, auditable, health-checkable
    #
    # timeout 5m: every limiter bucket here refills within ~2s of idle, so any
    # source idle longer than seconds carries semantically dead state. 5 minutes
    # bounds the set to the recent-activity population (dns1 measured worst case
    # ~0.2-5 new sources/s -> 60-1500 steady-state entries vs 65535 cap) while
    # adding zero enforcement change for any source active within the window.
    # AT CAPACITY (>65535 concurrently-active sources = massive DDoS): a NEW
    # source still takes the fallthrough drop — FAIL_CLOSED, deliberate, but now
    # self-healing via expiry and visible to `nftban health` (fill-ratio check).
    set syn_meter_v4 {
        type ipv4_addr
        size 65535
        flags dynamic,timeout
        timeout 5m
        comment "v1.228.6 bounded per-IP SYN rate state"
    }
    ...
    counter input_syn_rate_exceeded {
        comment "SYN not admitted — rate exceeded or non-service port"
    }

    counter input_service_tcp_accept {
        comment "Inbound TCP service port accepted (post-meter fallback)"
    }
    ...
    chain input {
        ...
        # v1.66.1 FIX: Added tcp dport @tcp_ports_in — SYN accept restricted to service ports.
        ...
        # v1.67.0: Log rate-limited to prevent disk saturation during scan storms.
        # Counters remain unlimited for accurate telemetry.
        tcp flags syn ct state new limit rate 10/second burst 20 packets log prefix "NFTBAN_PORTSCAN: " comment "SYN exceeded log (rate-limited)"
        tcp flags syn ct state new counter name input_syn_rate_exceeded counter name total_input_drop drop comment "SYN rate exceeded"
...
table ip6 nftban {
    ...
    # =========================================================================
    # BOUNDED RATE-LIMIT STATE (v1.228.6) — see the ip-table block for the full
    # rationale. v4/v6 parity is mandatory: ddos_dns_udp6 measured at 70-71% on
    # two resolvers, so a v4-only fix leaves the same outage latent on v6.
    # =========================================================================
    set syn_meter_v6 {
        type ipv6_addr
        size 65535
    ...
    # /64-aggregate SYN pressure. Prefix keys aggregate many hosts, so the key
    # population is smaller and each key summarises longer-lived behaviour:
    # 10m keeps aggregate visibility without meaningful state cost.
    # This is a `rate over … drop` limiter: at capacity a NEW prefix cannot
    # insert, the rule does not match, and traffic continues to later rules —
    # FAIL_OPEN (bounded protection loss, no outage), surfaced by health.
    set syn_prefix_meter_v6 {
        type ipv6_addr
        size 65535
        flags dynamic,timeout
        timeout 10m
        comment "v1.228.6 bounded /64 SYN-pressure state"
    }
    ...
    # v1.67.0: IPv6 /64 prefix SYN anti-rotation counter
    counter input_syn_prefix_drop {
        comment "IPv6 /64 prefix SYN rate exceeded (anti-rotation)"
    }
    ...
    chain input {
        ...
        # 5a. ICMPv6 error + echo — accept from any source
        ...
        # before the per-IP meter sees each address independently.
        # Uses limit rate over (inverse match) — drops only when /64 exceeds threshold.
        tcp flags syn ct state new tcp dport @tcp_ports_in update @syn_prefix_meter_v6 { ip6 saddr & ffff:ffff:ffff:ffff:: limit rate over 100/second burst 200 packets } counter name input_syn_prefix_drop counter name total_input_drop drop comment "/64 prefix SYN rate exceeded"

        # 7b. Per-IP SYN RATE — canonical fast-path admission
        # v1.46.0 FIX-B: Two-rule pattern.
        # v1.66.1 FIX: SYN accept restricted to service ports via @tcp_ports_in.
        tcp flags syn ct state new tcp dport @tcp_ports_in update @syn_meter_v6 { ip6 saddr limit rate 25/second burst 50 packets } counter name total_input_accept accept comment "SYN rate OK — service ports only"
        # v1.67.0: Log rate-limited to prevent disk saturation during scan storms.
        # Counters remain unlimited for accurate telemetry.
        tcp flags syn ct state new limit rate 10/second burst 20 packets log prefix "NFTBAN_PORTSCAN: " comment "SYN exceeded log (rate-limited)"
        tcp flags syn ct state new counter name input_syn_rate_exceeded counter name total_input_drop drop comment "SYN rate exceeded"
