...
for i in {1..6}; do
  ssh wronguser@lab.example.test
  sleep 2
done
```
...
# Check log
ssh root@lab.example.test "tail -5 /var/log/nftban/fail2ban-bans.log"
...
ssh root@lab.example.test "echo 'Auto-unban worked!'"
```
...
ssh root@lab.example.test "/usr/sbin/nftban-complete logs ip YOUR_IP"

# Recent bans
ssh root@lab.example.test "/usr/sbin/nftban-complete logs tail fail2ban-bans.log 50"

# JSON action log
ssh root@lab.example.test "/usr/sbin/nftban-complete logs tail nftban-actions.log 20"
```
...
```bash
ssh root@lab.example.test "tail -10 /var/log/nftban/fail2ban-bans.log"
```
...
```bash
# Check Fail2ban logs
ssh root@lab.example.test "tail -50 /var/log/fail2ban.log | grep nftban"

# Check jail status
ssh root@lab.example.test "fail2ban-client status nftban-sshd"

# Restart Fail2ban
ssh root@lab.example.test "systemctl restart fail2ban"
```

### **If bans don't appear in nftables:**
```bash
# Check runtime table exists
ssh root@lab.example.test "nft list table inet nftban_runtime"

# Check CLI logs
ssh root@lab.example.test "tail -20 /var/log/nftban/nftban-actions.log"

# Test manual ban
ssh root@lab.example.test "/usr/sbin/nftban-complete ban 1.2.3.4 --temp --timeout 60 --source test --jail manual"
```

### **If logs are empty:**
```bash
# Check directory permissions
ssh root@lab.example.test "ls -la /var/log/nftban/"

# Check log files
ssh root@lab.example.test "ls -lh /var/log/nftban/*.log"
```
...
**Test SSH ban:**
```bash
for i in {1..6}; do ssh wronguser@lab.example.test; sleep 2; done
```
...
**View logs:**
```bash
ssh root@lab.example.test "/usr/sbin/nftban-complete logs tail fail2ban-bans.log"
```

**View stats:**
```bash
ssh root@lab.example.test "/usr/sbin/nftban-complete stats top-ips"
```
