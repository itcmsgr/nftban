...
2. Try to update pin via SSH:
   ```bash
   ssh root@lab.mywebhost.gr "nftban update pin 2bb494131f28a893f10775977219b6a10e401cda"
   ```
3. Observe warnings but no success/failure message
4. Check pin file:
   ```bash
   ssh root@lab.mywebhost.gr "cat /etc/nftban/config/update-pins.conf"
   # Output: File not found or old SHA
   ```
5. Try to update:
   ```bash
   ssh root@lab.mywebhost.gr "nftban update perform"
...
# CentOS 9
ssh root@lab.mywebhost.gr "mkdir -p /etc/nftban/config && \
  echo '$NEW_SHA' > /etc/nftban/config/update-pins.conf && \
  chmod 600 /etc/nftban/config/update-pins.conf"

# Ubuntu 24.04
ssh root@lab1.mywebhost.gr "mkdir -p /etc/nftban/config && \
  echo '$NEW_SHA' > /etc/nftban/config/update-pins.conf && \
  chmod 600 /etc/nftban/config/update-pins.conf"

# CentOS 10
ssh root@65.21.157.15 "mkdir -p /etc/nftban/config && \
  echo '$NEW_SHA' > /etc/nftban/config/update-pins.conf && \
  chmod 600 /etc/nftban/config/update-pins.conf"
```
