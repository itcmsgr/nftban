...
```bash
# CentOS 9
ssh root@lab.example.test 'bash -s' < scripts/security_scan_lab.sh

# Ubuntu 24.04
ssh root@lab1.example.test 'bash -s' < scripts/security_scan_lab.sh

# CentOS 10
ssh root@198.51.100.15 'bash -s' < scripts/security_scan_lab.sh
```
...
ssh root@lab.example.test "ls -lt /var/log/nftban/security-scan-* 2>/dev/null"
ssh root@lab.example.test "ls -lt /tmp/security-scan-*.tar.gz 2>/dev/null"
