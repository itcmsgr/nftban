...
nftban_health_fix_auditor_acls() {
    ...
    if [[ -d ${NFTBAN_LOG_DIR} ]]; then
        setfacl -m g:nftban-auditor:x ${NFTBAN_LOG_DIR} 2>/dev/null || ((++acl_errors))
        for logfile in bans.log login_monitor.log feeds.log nftban-actions.log ddos.log portscan.log; do
            [[ -f "${NFTBAN_LOG_DIR}/$logfile" ]] && setfacl -m g:nftban-auditor:r "${NFTBAN_LOG_DIR}/$logfile" 2>/dev/null || true
        done
    fi
...
nftban_health_fix_permissions() {
    ...
    # ==========================================================================
    # This is the canonical way to fix /run/nftban, ${NFTBAN_LOG_DIR}, /var/cache/nftban
    ...
    if [[ $running_as_root -eq 1 ]]; then
        ...
        # ${NFTBAN_LOG_DIR} - logs written by nftban user
        if [[ -d ${NFTBAN_LOG_DIR} ]]; then
            chown nftban:nftban ${NFTBAN_LOG_DIR} 2>/dev/null || true
            chmod 750 ${NFTBAN_LOG_DIR} 2>/dev/null || true
        fi
    ...
    if [[ $running_as_root -eq 1 ]]; then
        ...
        if [[ ! -d "$suricata_dir" ]]; then
            mkdir -p "$suricata_dir" 2>/dev/null || return 1
        ...
        if [[ -d "$suricata_dir" ]]; then
            ...
            # This allows suricata to traverse ${NFTBAN_LOG_DIR}/ (owned by nftban:nftban)
...
EOF
    ...
    if [[ ! -f "$logrotate_dst" ]]; then
        if [[ -f "$logrotate_src" ]]; then
            ...
            # v1.137 (B-01): the canonical packaged template is the SINGLE source
            # of truth. This branch previously auto-generated a DIVERGENT policy
            # (a /*.log wildcard + a suricata stanza with `create 0640 suricata
            # nftban` that breaks when the suricata user is absent), drifting from
            # the packaged template. Do NOT emit a second policy — surface the
            # missing payload so the operator repairs the package instead.
            echo "  ⚠️  Canonical logrotate template not found at $logrotate_src" >&2
            echo "      Package payload incomplete — reinstall/repair the nftban package" >&2
            echo "      to restore /etc/logrotate.d/nftban (not auto-generating a divergent policy)." >&2
        fi
    else
    ...
    if [[ "$db_found" == "false" ]]; then
        ...
        if [[ ! -d "$geoip_dir" ]]; then
            ...
            install -d -o "${fhs_owner:-nftban}" -g "${fhs_group:-nftban}" -m "${fhs_perms:-0750}" "$geoip_dir" 2>/dev/null || {
                mkdir -p "$geoip_dir" 2>/dev/null || return 1
