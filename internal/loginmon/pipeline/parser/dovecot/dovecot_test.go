...
func TestParse_ImapAuthFailed(t *testing.T) {
	p := New()
	line := mkLine(`Apr  9 03:01:45 srv2 dovecot[3767044]: imap-login: Login aborted: Connection closed (auth failed, 1 attempts in 3 secs) (auth_failed): user=<contact@customer-a.example.test>, method=PLAIN, rip=34.138.135.178, lip=46.224.164.97, TLS, session=<21JEPP5Ozskiioey>`)
	r := p.Parse(line)
	...
	if r.Event.Username != "contact@customer-a.example.test" {
