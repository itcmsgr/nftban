#!/usr/bin/env bash
# =============================================================================
# NFTBan v0.32.20 - Unified Reporting Engine
# =============================================================================
...
declare -g NFTBAN_REPORT_ENGINE_VERSION="0.31.0"
...
nftban_report_init() {
    ...
    if ! declare -f nftban_log >/dev/null 2>&1; then
        if [[ -f "/usr/lib/nftban/core/nftban_base.sh" ]]; then
            source /usr/lib/nftban/core/nftban_base.sh
        fi
    fi
...
nftban_report_collect_module() {
    ...
    # Load module report
    if [[ -f "/usr/lib/nftban/core/nftban_report_module.sh" ]]; then
        source /usr/lib/nftban/core/nftban_report_module.sh
    else
...
nftban_report_collect_fhs() {
    ...
    # Load FHS report
    if [[ -f "/usr/lib/nftban/core/nftban_report_fhs.sh" ]]; then
        source /usr/lib/nftban/core/nftban_report_fhs.sh
    else
...
nftban_report_collect_services() {
    ...
    if [[ -f "/usr/lib/nftban/core/nftban_report_services.sh" ]]; then
        source /usr/lib/nftban/core/nftban_report_services.sh
    else
