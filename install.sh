#!/usr/bin/env bash
# =============================================================================
# NFTBan v1.9.3 - Installation Script
# =============================================================================
...
ask_metrics_question() {
    # In quiet mode, enable metrics by default (writes to files, no external backend required)
    if [[ "${NFTBAN_QUIET:-0}" == "1" ]]; then
...
install_logrotate() {
    ...
    if ! command -v logrotate &>/dev/null; then
        ...
        info "Install logrotate: apt install logrotate  OR  dnf install logrotate"
        return 0
    fi
    ...
    local log_rotate=4        # Default: 1 month
    local suricata_rotate=7   # Default: 7 days
    local report_rotate=3     # Default: 3 months

    # Get available disk space on /var (in GB)
    if [[ -d /var/log ]]; then
        avail_gb=$(df -BG /var/log 2>/dev/null | awk 'NR==2 {gsub(/G/,"",$4); print int($4)}' || echo "0")
    fi

    # Adjust rotation policy based on available space
    if [[ "$avail_gb" -lt 10 ]]; then
        # Small disk (<10GB): aggressive rotation
        log_frequency="daily"
        log_rotate=7         # 7 days
        suricata_rotate=3    # 3 days
        report_rotate=1      # 1 month
        info "Disk: ${avail_gb}GB (small) → rotation: logs 7d, Suricata 3d"
    elif [[ "$avail_gb" -lt 30 ]]; then
        # Medium disk (10-30GB): standard rotation (1 month)
        log_frequency="weekly"
        log_rotate=4         # 4 weeks (~1 month)
        suricata_rotate=7    # 7 days
        report_rotate=3      # 3 months
        info "Disk: ${avail_gb}GB (medium) → rotation: logs 1 month, Suricata 7d"
    else
        # Large disk (>30GB): extended rotation (2 months)
        log_frequency="weekly"
        log_rotate=8         # 8 weeks (~2 months)
        suricata_rotate=14   # 14 days
        report_rotate=6      # 6 months
        info "Disk: ${avail_gb}GB (large) → rotation: logs 2 months, Suricata 14d"
    fi

    # Generate logrotate config with calculated values
...
install_systemd() {
    ...
    # Feeds timer (only if feeds enabled - saves resources when not using threat intel)
...
if [[ -n "$DETECTED_PANEL" ]]; then
    ...
    # Security info: rpcbind exposure check
    ...
    if ss -lunpt 2>/dev/null | grep -q ':111 '; then
        echo ""
        warn "Security: Port 111 (rpcbind) exposed to internet"
        if ! systemctl is-active nfs-server.service &>/dev/null 2>&1; then
            info "Recommend disabling: systemctl disable --now rpcbind rpcbind.socket"
        else
            info "NFS detected. Recommend blocking: nftban port block 111"
        fi
    fi
fi
