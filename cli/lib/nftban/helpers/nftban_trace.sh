...
nftban_trace_stats() {
    ...
    echo ""
    echo "Total START entries: $(grep -c '\[START\]' "$trace_log" 2>/dev/null || true)"
    echo "Total END entries:   $(grep -c '\[END\]' "$trace_log" 2>/dev/null || true)"
    echo "Total ERROR entries: $(grep -c '\[ERROR\]' "$trace_log" 2>/dev/null || true)"
    echo "Total CHECK entries: $(grep -c '\[CHECK\]' "$trace_log" 2>/dev/null || true)"
    echo ""
