...
nftban_cmd_list() {
    ...
    if [[ "$list_type" == "banned" || "$list_type" == "all" ]]; then
        ...
        if nft list set ${NFTBAN_TABLE_IPV4} blacklist_ipv4 &>/dev/null; then
            ...
            done < <(parse_nft_set "$(nft list set ${NFTBAN_TABLE_IPV4} blacklist_ipv4 2>/dev/null)" "blacklist_ipv4")
        fi

        # Get blacklist_ipv6
        if nft list set ${NFTBAN_TABLE_IPV6} blacklist_ipv6 &>/dev/null; then
            while IFS= read -r ip; do
                [[ -n "$ip" ]] && all_ips["$ip"]="blacklist:ipv6"
            done < <(parse_nft_set "$(nft list set ${NFTBAN_TABLE_IPV6} blacklist_ipv6 2>/dev/null)" "blacklist_ipv6")
        fi
    fi

    if [[ "$list_type" == "whitelist" || "$list_type" == "all" ]]; then
        # v0.7.3: Get whitelist_ipv4
        if nft list set ${NFTBAN_TABLE_IPV4} whitelist_ipv4 &>/dev/null; then
            while IFS= read -r ip; do
                [[ -n "$ip" ]] && all_ips["$ip"]="whitelist:ipv4"
            done < <(parse_nft_set "$(nft list set ${NFTBAN_TABLE_IPV4} whitelist_ipv4 2>/dev/null)" "whitelist_ipv4")
        fi

        # Get whitelist_ipv6
        if nft list set ${NFTBAN_TABLE_IPV6} whitelist_ipv6 &>/dev/null; then
            while IFS= read -r ip; do
                [[ -n "$ip" ]] && all_ips["$ip"]="whitelist:ipv6"
            done < <(parse_nft_set "$(nft list set ${NFTBAN_TABLE_IPV6} whitelist_ipv6 2>/dev/null)" "whitelist_ipv6")
        fi
    fi
