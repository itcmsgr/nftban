...
# => project_rbl (self) = { 198.51.100.67, 2001:db8:c014:5ee1::1 } (2 addresses).
_ip_stub() {
  cat <<'STUB'
ip() {
  local j="$*"
  ...
  if [[ "$j" == *"-o addr show"* ]]; then
    printf '%s\n' \
      ...
      "2: eth0    inet 198.51.100.67/24 scope global eth0\\       valid_lft forever preferred_lft forever" \
...
o="$(run_server "host.example has address 198.51.100.67")"
