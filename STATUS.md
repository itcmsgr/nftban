...
**Version (this commit):** v1.104.0 — sourced from [`/VERSION`](VERSION); static per commit, not auto-updated. For the current released tag see [GitHub releases](https://github.com/itcmsgr/nftban/releases) or the README badge.
**Release lane:** v1.104.0 (2026-05-05) — focused Lane M test-guard release. One test-guard PR (PR-M-C #561, squash `67e7492f`) renames the temporary v1.103 Login boundary guard `TestLoadFromFile_LoginNotAliasedInC3C` into a permanent regression guard `TestLoadFromFile_LoginKeysAreDistinctFacts` and adds a symmetric overlay-path guard `TestOverlayFromFile_LoginKeysAreDistinctFacts`. **Lane M Decision B:** `LOGIN_ENABLED` (Go loginmon module self-enable; owned by `internal/loginmon`) and `NFTBAN_LOGIN_MONITOR_ENABLED` (central nftban infra observability gate; owned by `internal/nftbanconf`) are **distinct configuration facts** and must not be aliased. Drift row C0A-D-07 closed by formalizing the split. **No runtime behavior change.** Schema frozen at `1.83.0`. No metrics, portal, lifecycle, install, packaging, or panel-adapter changes. Issue #525 (geoip startup Go runtime panic, P1) remains open and is deferred to Lane G / v1.105 investigation; not introduced by v1.104 (carries over from v1.101 / v1.102 / v1.103 deferrals). Panel-adapter coverage from v1.103.0 unchanged.
**Schema status:** schema remains frozen at `1.83.0` in v1.104.0. No metric names, labels, or health JSON schema changed. The three v1.100.4 H4 disclosure items (ban attribution under `source=manual`, `EffectiveIdle` for Portscan/LoginMon, shared `input_syn_rate_exceeded` counter) are unchanged in v1.104.0. The schema / metrics lane (Lane S) remains held; deeper attribution / effective-state work is routed to a future release. No metrics, portal, lifecycle, install, packaging, or panel-adapter changes in v1.104.0.
