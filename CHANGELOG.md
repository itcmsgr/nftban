...
**Fix:** `release.yml` package jobs now explicitly use the verified **Mode-3** prebuilt path — `build_nftban.sh rpm|deb --use-prebuilt --prebuilt-manifest /workspace/bin/build-manifest.json` — consuming the single `go-binaries` artifact built once by the existing `build-binaries` job (no Go installed in the package containers, so DEB and RPM package byte-identical daemons by construction). Adds a manual **dry-run** surface (`workflow_dispatch`, `publish=false`) that runs the full build→package→assemble path and verifies the **pre-SLSA** release set this workflow owns — 7 packages, `nftband-linux-amd64`, SBOM, `SHA256SUMS.build`, MANIFEST, VERIFY, and DEB/RPM daemon parity (13 assets) — **without publishing**. `nftban-core-linux-amd64` and its `.intoto.jsonl` provenance are the two **downstream SLSA** assets (added by the SLSA workflow, which is gated on a successful Release Packages run), bringing the published total to 15; `SHA256SUMS` covers the packages + `nftband`, while `nftban-core` is covered by its SLSA provenance (deliberate separation). Adds a CI regression guard (`scripts/ci/check-build-provenance.sh`) that fails if either `release.yml` or `build-packages.yml` reintroduces a bare package invocation, with a negative self-test.

**Dry-run-surfaced prerequisite (jq):** the first post-merge executable dry-run (`workflow_dispatch`, `publish=false`) proved all 7 package jobs correctly entered Mode 3, then failed because `jq` — the mandatory `--use-prebuilt` manifest-verification dependency — was absent from the Go-less DEB/RPM containers (the old bare path never needed it). No package or release upload occurred. Fixed by installing `jq` (and `file`, for the RPM parity check) in the release.yml package containers, aligning them with the already-correct `build-packages.yml`. The CI regression guard was extended to assert the full Mode-3 contract per package job — `--use-prebuilt` + `--prebuilt-manifest` **and** the `jq` install, with the package containers installing no Go toolchain — anchored to each job's dependency-install command (a stray `jq` elsewhere in the YAML does not satisfy it) and backed by four negative self-tests (RPM/DEB jq removed, each Mode-3 flag removed). A second executable dry-run remains mandatory before publication.
