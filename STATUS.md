...
**Version (this commit):** v1.106.0 — sourced from [`/VERSION`](VERSION); static per commit, not auto-updated. For the current released tag see [GitHub releases](https://github.com/itcmsgr/nftban/releases) or the README badge.
**Release lane:** v1.106.0 (2026-05-06) — Lane MFST (Manifest Authority) partial release. Seven code PRs land manifest-pipeline drift closures: **C0a #563 (`8f5b35a8`)** wires RPM `%files` to consume the generator output `install/packaging/rpm/nftban-files.inc` via `%include`. **C0b #564 (`be0c9bc3`)** wires DEB `build_deb()` to consume `install/packaging/deb/nftban.dirs` via a while-read loop. **C1 #565 (`a07f4c2c`)** introduces `build/generate-systemd-install-list.sh` and a shared `nftban-systemd-install.list` (49 unit files); both packagers now consume the list, closing the prior 41-of-49 hand-rolled gap. The 8 net-new unit files ship passively but are file-presence only — no auto-enable; the Go installer continues to own enablement per PR-22B. **C2 #566 (`b331d5c3`)** regenerates `docs/systemd/UNITS.md` from filesystem truth (21 timers / 27 services / 1 socket) with CI count parity, splits Sockets into its own section, and corrects schedule and purpose details flagged by senior audit (rbl-check twice-daily + boot trigger; pro-inventory wording; +boot+Nm annotations on 8 timers). **C3 #567 (`ed632e34`)** introduces `build/deprecated-units.yaml` + `build/generate-systemd-maintainer-scripts.sh`; replaces hand-rolled RPM `%preun` and DEB `prerm` cleanup with generated symmetric snippets; the new `mask_if_exists()` helper guards mask/remove operations behind a `readlink == /dev/null` check, so clean hosts no longer accumulate `/etc/systemd/system/nftban-ui*` residue (Lane L10 F7_LOW class structurally closed) and operator-created custom symlinks at `/etc/systemd/system/$unit` are preserved. **C4 #568 (`63fab761`)** adds a Go test asserting `internal/installer/fhs/paths.go::RequiredDirs` is a subset of the generated FHS JSON (one inline exemption for `/var/lib/node_exporter/textfile_collector`). **C5 #569 (`4222f707`)** extends Go-side distro fixture coverage from 2 of 18 fixtures to all 18, asserting each parses cleanly and contains the seven shell-recognized sections. C6 (AM-4 + E6 structural CI hardening) is deferred to v1.107+ per operator queue narrowing. **No runtime behavior change beyond the C1 file-presence shift.** Schema frozen at `1.83.0`. No metrics, portal, install API, or panel-adapter changes. Issue #525 (geoip startup Go runtime panic, P1) remains open and is deferred to Lane G / v1.107+ investigation; not introduced by v1.106 (carries over from v1.101 / v1.102 / v1.103 / v1.104 deferrals). AUTH-HARDENING (D-NEW-8 / D-NEW-9 / D-NEW-12) and POLKIT-AUTHORITY (D-NEW-10 / D-NEW-11) lanes remain separately gated and not started in v1.106.
**Schema status:** schema remains frozen at `1.83.0` in v1.106.0. No metric names, labels, or health JSON schema changed. The three v1.100.4 H4 disclosure items (ban attribution under `source=manual`, `EffectiveIdle` for Portscan/LoginMon, shared `input_syn_rate_exceeded` counter) are unchanged in v1.106.0. The schema / metrics lane (Lane S) remains held; deeper attribution / effective-state work is routed to a future release. No metrics, portal, lifecycle, install, packaging-payload, or panel-adapter changes in v1.106.0.
