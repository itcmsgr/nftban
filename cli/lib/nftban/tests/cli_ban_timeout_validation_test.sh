...
# meta:description="v1.141 PR-A — asserts that `nftban ban X --timeout VALUE` rejects every non-positive-integer VALUE at parse time with rc=1 and a clear ERROR message, and accepts every positive integer (parse-time only — downstream daemon IPC is out of scope for this test). Prior to v1.141, the --timeout arm at cli/lib/nftban/cli/cmd_ban.sh:86-89 had no validation; values like 'abc' / '-5' / '0' / '' / '1.5' propagated to nftban-core which interpreted invalid values as permanent ban — operator surprise reproduced live in the cruel-judge review on monitor.example.test. Hermetic — runs against the source tree."
