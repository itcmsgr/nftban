...
func MeHandler(w http.ResponseWriter, r *http.Request) {
	...
	// TODO: Optimize health command or make it async
	var healthData map[string]interface{}
	// healthOutput, err := execNFTBanCommand("health", "--json")
	// if err == nil {
	...
	if firewall, ok := statusData["firewall"].(map[string]interface{}); ok {
		// Create counters object from firewall data
		userInfo["counters"] = map[string]interface{}{
			"blocked_total": firewall["banned_ips"],
			"rule_count":    firewall["rule_count"],
		}

		// Create active_sets array (mock for now - extract from nftables later)
	...
	if feedsData != nil {
		if data, ok := feedsData["data"].(map[string]interface{}); ok {
			...
			// Calculate feed deltas (mock data for now - would need to track over time)
			if enabledFeeds, ok := data["enabled_feeds"].([]interface{}); ok {
				deltas := make([]int, len(enabledFeeds))
				for i := range deltas {
					deltas[i] = 0 // TODO: track actual deltas
				}
				userInfo["feed_deltas"] = deltas
			}
		}
	} else {
		// Default values when feeds data unavailable
	...
	if healthData != nil {
		...
		// Use health status from basic status command
...
func BanHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Execute ban command using nftban CLI
	...
	// Check for success message in output - if found, ignore exit code errors
