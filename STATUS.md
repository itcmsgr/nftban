...
**In Release Prep:** v1.228.7 — stabilization (watchdog tiering, effective-config authority, geoban migration)
...
**v1.228.5 — the distro nftables service can read the ruleset NFTBan renders for it — `RELEASE_PREP`, NOT PUBLISHED** — *Implementation MERGED to `main` (PR #1182, squash `d2063fed`; PR head `2fa1a065`, tree `e06a9b73…` — `git diff 2fa1a065 d2063fed` EMPTY, so the accepted implementation did not change during squash merge). `V1_228_5_IMPLEMENTATION = MERGED` · `V1_228_5_RELEASE_PREP = IN_PROGRESS` · `V1_228_5_READY_TO_TAG = NO` · `V1_228_5_PUBLICATION = NOT_YET` · `V1_228_5_STANDALONE_FLEET_ROLLOUT = NOT_PLANNED` · `V1_228_5_FLEET_ROLLOUT_DISPOSITION = INTENDED_TO_BE_SUPERSEDED_BY_V1_228_6`. On stock EL9 with SELinux **Enforcing from first boot**, a clean install produced NO FIREWALL: the distro `nftables.service` executes `/sbin/nft` in `iptables_t` and must read `/etc/nftban/nftables.conf`, which carried `nftban_conf_t` — a type no rule permitted `iptables_t` to read — so the service failed and the install reported `INSTALL_STATE=FAILED_NO_FIREWALL`. The rendered ruleset now carries a dedicated `nftban_nftables_conf_t`, granting `iptables_t` exactly one file rather than NFTBan's configuration surface; the publish path establishes that type so `rebuild --force` and `restorecon -RFv` cannot lose it. Recorded design fact: an **`allowxperm` REFINES an already-authorised base permission and cannot grant one** — an xperm-only first attempt compiled cleanly and was refuted at runtime by a live EL10 `avc: denied { ioctl } comm="nft" ioctlcmd=0x5401`. `PRODUCT_CODE_CHANGE = shell + SELinux policy + packaging` · `DAEMON_CHANGE = NO` · `SCHEMA_CHANGE = NO`. **Required gates before publication: ordinary CI and the trusted/private gate on the exact release-prep SHA, then DEB built on lab2 and EL9 RPM built on tgt-a9 (`lab4` is PROHIBITED as a build authority following the 2026-07-29 root-compromise finding; lab2 cannot build the RPM), with package/version/payload identity and a real `1.228.4 → 1.228.5` upgrade transaction for both families.** Latent and explicitly NOT closed: the SELinux install-time controls (`semodule -i` without a three-state `getenforce` branch, without a `semodule -l` load verification, and with no CI regression guard).*

**v1.228.4 — a failed read is no longer proof of absence — RELEASED (2026-07-30), NOT FLEET-DEPLOYED** — *tag `v1.228.4` → `6bde73f8`, 15 assets, public/non-draft. `FLEET = NOT_STARTED` — publication is not deployment. Three PRs merged to `main` (#1178 `3d1be747` nft consumer authority inventory + guard · #1179 `30900f04` AF_NETLINK correction across six units · #1180 `18eafc40` typed nft probe with fail-closed absence and a monotonic degraded latch). `TAG = v1.228.4 → 6bde73f8` · `PUBLICATION = COMPLETE (2026-07-30, 15 assets)` · `FLEET = NOT_STARTED`. Governing invariant: a failed read is not evidence of absence, and only a VERIFIED absence may authorise a mutation. Six units sandboxed away `AF_NETLINK`, so every `nft` call inside them failed `EAFNOSUPPORT` and the "firewall table absent" message ran ~927 times per host while both tables existed; `nftban-queue` and `nftban-botscan` deliberately still WITHHOLD `AF_NETLINK`. The probe now yields `PRESENT | ABSENT | CANNOT_READ`, establishes readability POSITIVELY, and latches degradation monotonically so a later success cannot erase an earlier blindness. `PRODUCT_CODE_CHANGE = shell + systemd units + CI` · `DAEMON_CHANGE = NO` · `SCHEMA_CHANGE = NO`. Package-native upgrade validation v1.228.2 → v1.228.4 on lab2 (DEB) and lab4 (EL9/RPM) was the publication gate and was satisfied before tagging; the two-lab 29/29 negative-control proof used a file overlay, not installed packages. SELinux Enforcing remains UNTESTED (both labs Permissive/SELinux-less); the EL9 Enforcing clean-install blocker is the v1.228.5 lane.*

**v1.228.2 — CLI truth: a command that cannot do its work no longer reports success — RELEASED (2026-07-28), NOT FLEET-DEPLOYED** — *tag `v1.228.2` → `7f846d7d`, 15 assets, public/non-draft. `FLEET = NOT_STARTED`. Four PRs merged to `main` (#1170 `081ffb71` Suricata retirement · #1171 `d0289184` router exit truth · #1172 `f2042f9d` command-count authority + advertising parity · #1173 `3be2ed09` snapshot dispatch truth). `TAG = v1.228.2 → 7f846d7d` · `PUBLICATION = COMPLETE (2026-07-28, 15 assets)` · `FLEET = NOT_STARTED`. **`v1.228.1` was never published** — the lane originally numbered `.1` is deferred, because publishing it after `.2` would present as a downgrade to dpkg/rpm version comparison. Governing invariant: a component that failed to perform its required work must not report success. The router returned `0` when it had nothing to run (8 library files dispatchable, exiting 0 silently); `nftban snapshot` created snapshots when asked to list, restore or help, including hourly in production via `nftban-snapshot.service`. Suricata is retired to a dormant placeholder — 3 units removed from both package families, public surface reduced to `status`+`help`, registry narrowed 42→2 subcommands, 26 wrong or dead completion tokens removed. Four guards that measured the wrong thing were corrected: a count guard pinning a literal that punished correctness, an allowlist matching comment prose, a phantom-subcommand blind spot across all five existing guards, and a systemd-token gap. `PRODUCT_CODE_CHANGE = shell + CI only` · `DAEMON_CHANGE = NO` · `SCHEMA_CHANGE = NO` (nft 1.84.0 / config 1.1.0) · no exit-code values renumbered. **Required gate before publication: upgrade from v1.228.0 on lab2 (DEB) and lab4 (EL9/RPM) with legacy NFTBan Suricata units ENABLED** — proving the units are stopped, disabled and removed with no restart loop and upstream Suricata untouched. Final `main` tree verified byte-identical to the validated cumulative tree.*

**v1.228.0 — Post-install outcome truth: the package boundary reports whether the install actually succeeded — RELEASED (2026-07-27), FLEET-LIVE 9/9** — *Implementation MERGED to `main` (PR #1166, squash `4676480f`). `TAG = v1.228.0 → 0c2a0d9a` · `PUBLICATION = COMPLETE (2026-07-27, 15 assets)` · `FLEET = 9/9 production hosts — this is the CURRENT production baseline`. DEB `postinst` and RPM `%post` now run a read-only verifier after every installer outcome and emit machine-readable tokens, so a package manager printing `Complete!` over a failed install is distinguishable from a verified one. Ledger: `PACKAGE_POSTINSTALL_OUTCOME_TRUTH = MUTATION_PROVEN` (T8 44/44 on both DEB and RPM against the real verifier and the installed binary, outcome parity PASS). Explicitly **NOT** promoted: `PACKAGE_PRETRANSITION_T9_BOUNDARY = NOT_YET_VERIFIED` (no shipped injection hook; the lifecycle L5 case remains an analogue, not a proof) and `LAB4_SELINUX_ENFORCING = NOT_VALIDATED` (lab4 SELinux is Disabled; the v1.226 waiver stands — the separate stock-Enforcing tgt-a9 result is different host coverage and must not be restated as a lab4 Enforcing PASS). Post-merge CI attempt 1 recorded 2 non-required failures (`Fuzz Tests` = flaky 30s fuzz-deadline with no reproducer, first seen 18 days earlier on a metadata-only commit; `Build Docker Image` = ghcr.io login transport timeout); both classified with evidence and PASSED on an exact-SHA rerun with no source mutation — the failed attempts are retained as permanent history, not erased. Remaining confirmed runtime-mode defects are recorded in `docs/MODE_ADMISSION_LEDGER.md` and are **not** corrected in this release.*
