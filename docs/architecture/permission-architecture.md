...
## 📋 DYNAMIC UID/GID CONFIGURATION

### No Static IDs - System Auto-Assignment

NFTBan does NOT use static UID/GID values. Each system auto-assigns IDs during installation.

**Why dynamic?**
- ✅ No conflicts with existing system users
...
After installation, the assigned IDs are saved to:
```
/var/lib/nftban/config/system.conf
```
...
- Must stay synchronized with actual system UID/GID
- Health check will detect mismatches
- Use `sudo nftban health fix all` to regenerate if needed

**Usage:**
```bash
# Scripts can source this file if needed
source /var/lib/nftban/config/system.conf
echo "NFTBan user UID: ${NFTBAN_UID}"

# Verify alignment with system
[[ $(id -u nftban) -eq $NFTBAN_UID ]] && echo "Aligned" || echo "MISMATCH!"
```
