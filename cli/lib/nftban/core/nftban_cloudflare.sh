#!/usr/bin/env bash
# =============================================================================
# NFTBan v1.0.0 - Cloudflare Core Module
# Part of NFTBan v1.0.0 - FHS-Compliant Architecture
# =============================================================================
...
# =============================================================================
# WRITE TO WHITELIST FILE (v0.6.0)
# =============================================================================
...
EOF
    ...
    # Set correct permissions so autoheal/periodic sync can read it
    chmod 640 "$CF_WHITELIST_FILE"
    chown root:nftban "$CF_WHITELIST_FILE" 2>/dev/null || true

    local total
    total=$({ grep -v "^#" "$CF_WHITELIST_FILE" | grep -v "^$" || true; } | wc -l)

    _cf_log "INFO" "Wrote $total Cloudflare IP ranges to whitelist"
    _cf_log "INFO" "Whitelist will be synced to nftables by autoheal mechanism"
...
nftban_cloudflare_enable() {
    ...
    # Trigger system whitelist rebuild (v0.6.0)
