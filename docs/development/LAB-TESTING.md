...
This guide is specifically for testing NFTBan RPM packages on your production lab servers (lab.example.test, lab1.example.test, lab2.example.test).
...
- lab.example.test: nftban=994, nftban-cli=993
- lab1.example.test: nftban=994, nftban-cli=993
- lab2.example.test: nftban=993, nftban-cli=992
...
```bash
# From dev machine
scp dist/packages/nftban-0.10.0-1.el9.x86_64.rpm root@new-lab.example.test:/tmp/
