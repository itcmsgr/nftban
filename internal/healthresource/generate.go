...
func Render(p safety.HealthResourceProfile, sourceVersion string) []byte {
	...
	// STABLE facts only — NO avail_ram (volatile: it fluctuates between reconciles
	// and would break no-churn, causing a rewrite + daemon-reload every run).
	fmt.Fprintf(&b, "# tier=%s total_ram_bytes=%d cpu_cores=%d clamped=%t\n",
		p.Tier, p.TotalRAM, p.CPUCores, p.Clamped)
