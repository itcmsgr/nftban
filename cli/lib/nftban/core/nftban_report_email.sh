...
nftban_report_email_generate() {
    ...
    if [[ -f "$login_alert_log" ]]; then
        ...
        login_bans=$(grep -a "Banning IP" "$login_alert_log" 2>/dev/null | grep -a -E "^\[${yesterday}|^\[${today}" | wc -l || true)
    ...
    if [[ -f "$portscan_log" ]]; then
        portscan_bans=$(grep -a -Ei "ban|block" "$portscan_log" 2>/dev/null | grep -a -E "^\[${yesterday}|^\[${today}" | wc -l || true)
    ...
    if [[ -f "$ddos_log" ]]; then
        ddos_bans=$(grep -a -Ei "ban|block" "$ddos_log" 2>/dev/null | grep -a -E "^\[${yesterday}|^\[${today}" | wc -l || true)
