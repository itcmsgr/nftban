...
This checklist is for testing NFTBan v0.10.0 **from scratch** on a brand new, clean lab server (lab4.example.test) to verify:
...
```bash
# Copy package
scp dist/packages/nftban-0.10.0-1.el9.x86_64.rpm root@lab4.example.test:/tmp/

# Copy checksum (optional)
scp dist/packages/SHA256SUMS root@lab4.example.test:/tmp/
```
