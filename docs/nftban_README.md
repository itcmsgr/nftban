...
| Set       | set   | system_blacklist_v4| (impl.)| IPv4 bulk bans (country blocks) |
| Set       | set   | user_blacklist_v4  | (impl.)| IPv4 manual user bans |
| Set       | set   | temp_ban_v4        | (impl.)| IPv4 temporary bans |
| Chain     | chain | input              | (impl.)| Processes incoming traffic (whitelist → blacklist → accept/drop) |

The `nftban_global` table is the **core firewall structure**.  
`system_blacklist_v4` is a **Set within that table**, not a table itself.
