...
# Version: 3.0.2
# Author: ITCMS Team (Antonios Voulvoulis) - modifications for temp ban set references
# Description:
# This script automates the management for both nftables and fail2ban from one interface
# Temporary bans now reference temp_ban_v4 and temp_ban_v6 only.
################################################################################
...
# Function to verify the global table exists (created by init_nftables script)
ensure_temp_ban_table() {
    ...
    if ! nft list table inet "$TEMP_BAN_TABLE" >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW}Please run nftban_init_nftables_conf.sh first to create the table structure.${NC}"
        return 1
    fi

    # Verify IPv4 set exists
...
temp_ban_ip() {
    ...
    # Add to the appropriate set only (v4 or v6)
    if [[ "$ip" =~ .*:.* ]]; then
        # IPv6
...
remove_temp_ban() {
    ...
    # Remove from appropriate set only
    if [[ "$ip" =~ .*:.* ]]; then
        # IPv6
...
list_temp_bans() {
    ...
    # Explicitly list elements from the IPv4 set
    if nft list set inet "$TEMP_BAN_TABLE" "$TEMP_BAN_SET_V4" 2>/dev/null | grep -q "elements"; then
        # Print elements, support single-line or multi-line output
        nft list set inet "$TEMP_BAN_TABLE" "$TEMP_BAN_SET_V4" 2>/dev/null | sed -n '1,200p' | grep -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}(/[0-9]{1,2})?' || echo "None"
    else
        echo "None"
    fi
    ...
    if nft list set inet "$TEMP_BAN_TABLE" "$TEMP_BAN_SET_V6" 2>/dev/null | grep -q "elements"; then
        nft list set inet "$TEMP_BAN_TABLE" "$TEMP_BAN_SET_V6" 2>/dev/null | sed -n '1,200p' | grep -Eo '([0-9a-fA-F]{0,4}:){2,7}[0-9a-fA-F]{0,4}(/[0-9]{1,3})?' || echo "None"
    else
        echo "None"
    fi
}
...
# Function to view all banned IPs (explicitly from temp_ban sets and blacklist files + Fail2Ban)
view_all_banned() {
    echo -e "${BLUE}=== NFTables: Temporary bans (from sets) ===${NC}"

    # IPv4 temp bans from the explicit set
    ...
    # IPv6 temp bans from the explicit set
    ...
    if [ -f "$IPV4_BLACKLIST_FILE" ]; then
        ...
        grep -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}(/[0-9]{1,2})?' "$IPV4_BLACKLIST_FILE" || echo "None"
    else
    ...
    if [ -f "$IPV6_BLACKLIST_FILE" ]; then
        ...
        grep -Eo '([0-9a-fA-F]{0,4}:){2,7}[0-9a-fA-F]{0,4}(/[0-9]{1,3})?' "$IPV6_BLACKLIST_FILE" || echo "None"
    else
...
remove_ip_from_all() {
    ...
    # Remove from nftables temporary ban (explicit sets)
