...
**Order:**
1. Deploy to lab2.example.test (test server)
2. Test for 30 minutes
3. Deploy to lab.example.test
4. Deploy to lab1.example.test
...
```bash
# Test on lab2.example.test (test server)
...
```bash
# Test on lab2.example.test
# From your local machine, trigger 5 failed SSH attempts:

for i in {1..6}; do
  ssh wronguser@lab2.example.test 2>/dev/null || true
  sleep 2
done
