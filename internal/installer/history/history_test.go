...
func TestWriteEntry_NewFile(t *testing.T) {
	...
	entry := NewEntry("1.72.0", "1.73.0", StatusSuccess, "rpm", 45, "lab4.example.test")
	...
	if entries[0].Host != "lab4.example.test" {
		t.Errorf("Host = %s, want lab4.example.test", entries[0].Host)
	}
}
...
func TestWriteEntry_Prepend(t *testing.T) {
	...
	// Write first entry
	e1 := NewEntry("1.71.0", "1.72.0", StatusSuccess, "rpm", 30, "lab.example.test")
	if err := WriteEntry(path, e1); err != nil {
		t.Fatalf("WriteEntry 1: %v", err)
	}

	// Write second entry (should be prepended)
	e2 := NewEntry("1.72.0", "1.73.0", StatusVerifyFail, "rpm", 55, "lab4.example.test")
...
func TestWriteEntry_ShellCompatibleJSON(t *testing.T) {
	...
	e := NewEntry("1.72.0", "1.73.0", "success", "rpm", 45, "lab4.example.test")
	if err := WriteEntry(path, e); err != nil {
		t.Fatalf("WriteEntry: %v", err)
	}
