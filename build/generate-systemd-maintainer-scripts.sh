...
#
# Outputs:
#   install/packaging/rpm/nftban-preun-systemd-cleanup.inc  (RPM %preun body)
#   install/packaging/deb/nftban-prerm-systemd-cleanup.inc  (DEB prerm sentinel-region body)
#
...
generate_all() {
    ...
    if [[ "$rpm_t" = "$RPM_OUT" && "$deb_t" = "$DEB_OUT" ]]; then
        # Only update sentinel in real run, not check-mode temp dirs
