...
install_logrotate() {
    ...
    local logrotate_src="$SCRIPT_DIR/install/config/nftban.logrotate"
    ...
    # ==========================================================================
    # FLAT ROTATION POLICY (simple, predictable)
    # - Main logs: 1 month (weekly × 4)
    # - Suricata: 7 days (daily × 7) - higher volume
    # - Reports: 3 months (monthly × 3)
    # ==========================================================================
    ...
    if [[ -f "$logrotate_src" ]]; then
        ...
        info "Policy: main logs 1 month, Suricata 7 days, reports 3 months"
    else
        # Create logrotate config if source not found
        cat > "$logrotate_dst" << 'LOGROTATE'
# NFTBan Log Rotation (auto-generated by installer)
# Policy: main logs 1 month, Suricata 7 days, reports 3 months
...
LOGROTATE
        chown root:root "$logrotate_dst"
        chmod 0644 "$logrotate_dst"
        ok "Logrotate config created: $logrotate_dst"
        info "Policy: main logs 1 month, Suricata 7 days, reports 3 months"
    fi
