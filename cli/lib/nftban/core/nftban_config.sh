...
nftban_config_load() {
    ...
    if nftban_config_is_ini "$file"; then
        ...
        # config file path determined by format detection
...
nftban_config_set() {
    ...
    # v1.59.1 TOCTOU: Atomic create — temp+chmod+mv (prevents window with wrong perms)
    if [[ ! -f "$NFTBAN_LOCAL_CONF" ]]; then
        {
            echo "# NFTBan Local Configuration Overrides"
            echo "# Auto-generated by nftban config command"
            echo ""
        } > "${NFTBAN_LOCAL_CONF}.tmp"
        chmod 640 "${NFTBAN_LOCAL_CONF}.tmp"
        mv -f "${NFTBAN_LOCAL_CONF}.tmp" "$NFTBAN_LOCAL_CONF"
    fi
