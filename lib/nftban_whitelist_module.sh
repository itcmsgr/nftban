...
# =============================================================================
# NFTBan Whitelist Module - Production-Hardened (v0.9.3+)
# Version: 0.9.3
...
# - ✅ BUG61 FIX: Safe arithmetic assignments (strict mode compatible)
# - ✅ Auto-protection: Server IPs, localhost, current user
# - ✅ Split table architecture: System/User/Cloudflare whitelists
#
...
_nftban_whitelist_on_err() {
    ...
    if declare -f nftban_log_error >/dev/null 2>&1; then
        nftban_log_error "WHITELIST MODULE ERROR in ${func} at line ${line}; exit status ${rc}"
    else
        echo "ERROR: WHITELIST MODULE in ${func} at line ${line}; exit status ${rc}" >&2
    fi
...
trap '_nftban_whitelist_on_err ${LINENO} ${FUNCNAME[0]:-main}' ERR
