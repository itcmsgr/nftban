...
**Version (this commit):** v1.108.0 — sourced from [`/VERSION`](VERSION); static per commit, not auto-updated. For the current released tag see [GitHub releases](https://github.com/itcmsgr/nftban/releases) or the README badge.
...
**Schema status:** schema remains frozen at `1.83.0` in v1.108.0. No metric names, labels, or health JSON schema changed. The three v1.100.4 H4 disclosure items (ban attribution under `source=manual`, `EffectiveIdle` for Portscan/LoginMon, shared `input_syn_rate_exceeded` counter) are unchanged. The schema / metrics lane (Lane S) remains held; deeper attribution / effective-state work is routed to a future release. No metrics, portal, install API, or panel-adapter changes in v1.108.0; behavior change is confined to the CLI install-method classifier (`cli/lib/nftban/cli/cmd_update_detection.sh`) and the installer state-writer terminal hygiene (`internal/installer/state/file.go::Transition`).
