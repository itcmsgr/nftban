...
# STATE, not a packaged file — it is generated in %post from the shipped template
# (or copied verbatim from the template as a fail-safe). It is therefore NOT
# installed into the payload here and is declared %ghost in %files, so neither
...
# `rpm -V` NEVER reports it — no verification noise. It is created in %post by
