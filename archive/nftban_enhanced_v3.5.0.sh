...
show_help() {
  cat << 'EOF'
NFTBan – nftables & Fail2Ban helper (v3.5.0)
===========================================
...
main() {
  ...
  case "$arg1" in
    ...
    # Add to active whitelist set
    if [[ "$arg2" =~ .*:.* ]]; then
        nft add element inet "$TEMP_BAN_TABLE" "$WHITELIST_SET_V6" { "$arg2" }
    else
        nft add element inet "$TEMP_BAN_TABLE" "$WHITELIST_SET_V4" { "$arg2" }
    fi
    ...
    # Remove from active whitelist set
    if [[ "$arg2" =~ .*:.* ]]; then
        nft delete element inet "$TEMP_BAN_TABLE" "$WHITELIST_SET_V6" { "$arg2" }
    else
        nft delete element inet "$TEMP_BAN_TABLE" "$WHITELIST_SET_V4" { "$arg2" }
    fi
