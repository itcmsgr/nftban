...
**Version (this commit):** v1.103.0 — sourced from [`/VERSION`](VERSION); static per commit, not auto-updated. For the current released tag see [GitHub releases](https://github.com/itcmsgr/nftban/releases) or the README badge.
**Release lane:** v1.103.0 (2026-05-05) — focused Lane C / C3-C config canonical-alias release. One code PR (PR-C3-C #559, squash `f6163af6`) adds read-side aliases for `NFTBAN_DDOS_ENABLED` → `DDOS_ENABLED` and `NFTBAN_PORTSCAN_ENABLED` → `PORTSCAN_ENABLED` (Strategy C: read both, canonical wins on conflict, warn on conflict). Drift rows C0A-D-05 (DDoS) and C0A-D-06 (Portscan) closed. Login (C0A-D-07) deferred to v1.104 Lane M. BotGuard (C0A-D-08) skipped — production clean; regression-guard fixture retained. GeoBan tracked separately as GEO-CLEANUP-001. Issue #525 (geoip startup panic, P1) remains open and is deferred to v1.104 investigation; not introduced by v1.103. Panel-adapter coverage from v1.102.0 unchanged.
**Schema status:** schema remains frozen at `1.83.0` in v1.103.0. No metric names, labels, or health JSON schema changed. The three v1.100.4 H4 disclosure items (ban attribution under `source=manual`, `EffectiveIdle` for Portscan/LoginMon, shared `input_syn_rate_exceeded` counter) are unchanged in v1.103.0. The schema / metrics lane (Lane S) remains held; deeper attribution / effective-state work is routed to a future release. No metrics, portal, lifecycle, install, packaging, or panel-adapter changes in v1.103.0.
