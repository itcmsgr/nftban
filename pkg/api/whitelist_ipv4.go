...
func (api *API) HandleWhitelistIPv4Batch(w http.ResponseWriter, r *http.Request) {
	...
	// Apply changes
	if err := sync.ApplyStringDiffToSet(api.NFT, api.WhitelistIPv4Set, diff); err != nil {
		respondError(w, http.StatusInternalServerError, "failed to update whitelist: "+err.Error())
		return
	}
	...
	resp := BatchResponse{
		...
		Success: true,
		Message: "Whitelist IPv4 updated successfully",
	}
...
func (api *API) HandleWhitelistIPv4Add(w http.ResponseWriter, r *http.Request) {
	...
	resp := SingleIPResponse{
		Success: true,
		Message: "IP " + req.IP + " added to whitelist",
	}
...
func (api *API) HandleWhitelistIPv4Remove(w http.ResponseWriter, r *http.Request) {
	...
	resp := SingleIPResponse{
		Success: true,
		Message: "IP " + req.IP + " removed from whitelist",
	}
...
func (api *API) HandleWhitelistIPv4Preview(w http.ResponseWriter, r *http.Request) {
	...
	// Get current state
	current, err := api.NFT.GetSetElements(api.WhitelistIPv4Set)
	if err != nil {
