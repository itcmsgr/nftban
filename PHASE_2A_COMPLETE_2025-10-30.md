...
$ ssh root@lab.mywebhost.gr 'nft list set inet nftban_runtime temp_ban_v4 | grep -c timeout'
4

$ ssh root@lab.mywebhost.gr 'nft list set inet nftban_runtime temp_ban_v6 | grep -c timeout'
1

$ ssh root@lab.mywebhost.gr 'nft list set inet nftban_main whitelist_v4 | grep -oP "\d+\.\d+\.\d+\.\d+" | wc -l'
2

$ ssh root@lab.mywebhost.gr 'nft list set inet nftban_main whitelist_v6 | grep -c "::"'
1
...
table inet nftban_main {
	set whitelist_v4 {
		type ipv4_addr
		elements = { 62.38.150.122, 95.216.159.238 }
	}
}
