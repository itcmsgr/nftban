...
func (m *Module) loadConfig() {
	...
	// DDOS_CLASSIC_* and DDOS_SURICATA_* keys populate the SAME struct
...
func (m *Module) loadModeConfig(rootConfigDir, runDir, moduleConfigDir string) {
	effective, basis := nftbanconf.ReadEffectiveMode("ddos", rootConfigDir, runDir)
	m.modeConfigSource = ""
	...
	default: // unknown
		log.Printf("[ddos] effective mode UNKNOWN (%s) -- loading NO mode-specific config; "+
...
func (m *Module) Start(ctx context.Context) error {
	...
	{
		cfg := nftbanconf.MustLoad()
		m.loadModeConfig(cfg.ConfigDir, cfg.RunDir, filepath.Join(cfg.ConfigDir, "conf.d", "ddos"))
		if m.modeConfigSource == "" {
			log.Printf("[ddos] no mode-specific config loaded after reconcile (%s)", m.modeConfigBasis)
		}
	}
