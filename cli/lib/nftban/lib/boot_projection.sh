...
nftban_boot_projection_generate() {
    ...
    case "$rc" in
        0) : ;;
        2)
            # ⛔ UNKNOWN MUST NOT AUTHORIZE NEW BOOT AUTHORITY.
            #
            # The publication contract is:
            #   rc=0  valid                        -> publication PERMITTED
            #   rc=1  invalid                      -> publication FORBIDDEN
            #   rc=2  validity CANNOT BE ESTABLISHED
            #           existing == candidate      -> no mutation required, no-op OK
            #           existing != candidate      -> publication FORBIDDEN
            #           existing absent            -> publication FORBIDDEN
            #
            # UNKNOWN may PRESERVE an already-established identical artifact. It may
            # never CREATE or CHANGE one. "No previous projection exists" does not
            # make an unverified candidate safe -- it only means there is nothing to
            # compare it against, which is strictly less evidence, not more.
            #
            # MEASURED FAIL-OPEN, v1.229.13 Lane 3B. This branch previously installed
            # unconditionally with a warning. Where `nft -c` cannot run at all -- nft
            # not installed, which is the CI runner -- EVERY candidate is UNKNOWN, so
            # a syntactically INVALID projection silently replaced a known-good one.
            # Reproduced exactly: forcing this rc turned fpa_render_boot from
            # pass=12 fail=0 into pass=10 fail=2 on the two CI messages verbatim.
            #
            # This matters more after the distro include is repointed at this
            # artifact (Lane 3D): today the legacy include still cushions a bad
            # projection; afterwards this path is boot-critical.
            if [[ ! -s "$out" ]]; then
                echo "[NFTBan ERROR] boot projection: validity could NOT be established and there is NO existing projection — REFUSING to publish" >&2
                echo "[NFTBan ERROR]   an unverified artifact must not become the boot authority" >&2
                echo "[NFTBan ERROR]   install nftables (nft) so the candidate can be checked" >&2
                return 1
            fi
            if ! cmp -s "$tmp_out" "$out"; then
                echo "[NFTBan ERROR] boot projection: validity could NOT be established and the candidate DIFFERS from the existing projection — REFUSING to replace it" >&2
                echo "[NFTBan ERROR]   existing: $out" >&2
                echo "[NFTBan ERROR]   install nftables (nft) so the candidate can be checked, or remove the existing projection deliberately" >&2
                return 1
            fi
