...
# Version: 0.5.1 (COMPLETE - With all delegation functions added)
...
#   NOW INCLUDES ALL MISSING DELEGATION FUNCTIONS.
# =============================================================================
set -euo pipefail
...
log_ban_attempt() {
    local ip="$1"
    local jail_name="$2"
    local action="$3"  # BANNED, WHITELISTED, ALREADY_EXISTS, ERROR, PERMANENT_BAN, UNBANNED
    local reason="$4"
...
geoip_lookup() {
    ...
    if [ -z "$result" ] && command -v curl &> /dev/null; then
        result=$(curl -s "http://ip-api.com/json/${ip}?fields=status,countryCode,city,isp" 2>/dev/null | \
...
# NEW DELEGATION FUNCTION: Add IP to whitelist
add_ip_to_whitelist() {
    local ip="$1"
    local comment="${2:-Added via nftban on $(date +'%Y-%m-%d %H:%M:%S')}"
    ...
    if check_ip_in_whitelist_files "$ip"; then
        log_warning "IP $ip is already in whitelist"
        return 0
    fi
    if [ ! -f "$USER_WHITELIST_FILE" ]; then
        cat > "$USER_WHITELIST_FILE" << 'EOF'
...
EOF
    ...
    echo "${ip}  # ${comment}" >> "$USER_WHITELIST_FILE"
    log_success "Added $ip to user whitelist file"
    if check_nftables_table_exists; then
        if nft add element inet "$NFT_TABLE" "whitelist_v${ver}" "{ $ip }" 2>/dev/null; then
            log_success "Added $ip to nftables whitelist_v${ver}"
        fi
    fi
    create_consolidated_whitelist
    build_consolidated_search_file
...
# NEW DELEGATION FUNCTION: Remove IP from whitelist
remove_ip_from_whitelist() {
    local ip="$1"
    log_info "Removing IP from whitelist: $ip"
    local ver
    ver=$(detect_ip_version "$ip")
    if [ "$ver" == "invalid" ]; then
        log_error "Invalid IP address: $ip"
    ...
    local removed=false
    for file in "$USER_WHITELIST_FILE" "$SYSTEM_WHITELIST_FILE" "$FAIL2BAN_WHITELIST"; do
        if [ -f "$file" ]; then
            if grep -qE "^${ip}[[:space:]]" "$file" 2>/dev/null; then
                sed -i "/^${ip}[[:space:]]/d" "$file"
                log_success "Removed $ip from $(basename "$file")"
                removed=true
            fi
        fi
    done
    if check_nftables_table_exists; then
        if nft delete element inet "$NFT_TABLE" "whitelist_v${ver}" "{ $ip }" 2>/dev/null; then
            log_success "Removed $ip from nftables whitelist_v${ver}"
            removed=true
        fi
    fi
    if [ "$removed" = true ]; then
        create_consolidated_whitelist
        build_consolidated_search_file
        log_success "IP $ip removed from whitelist"
        return 0
    else
        log_warning "IP $ip not found in any whitelist"
        return 1
    fi
}
# NEW DELEGATION FUNCTION: Check if IP is whitelisted (for exit code checking)
check_ip_whitelisted() {
    local ip="$1"
    check_ip_in_whitelist_files "$ip"
}
# =============================================================================
...
# NEW DELEGATION FUNCTION: Remove IP from blacklist files
remove_ip_from_blacklist() {
    local ip="$1"
    log_info "Removing IP from blacklist: $ip"
    local ver
    ver=$(detect_ip_version "$ip")
    if [ "$ver" == "invalid" ]; then
        log_error "Invalid IP address: $ip"
    ...
    if check_nftables_table_exists; then
        if nft delete element inet "$NFT_TABLE" "user_blacklist_v${ver}" "{ $ip }" 2>/dev/null; then
            log_success "Removed $ip from nftables user_blacklist_v${ver}"
            removed=true
        fi
        if nft delete element inet "$NFT_TABLE" "system_blacklist_v${ver}" "{ $ip }" 2>/dev/null; then
            log_success "Removed $ip from nftables system_blacklist_v${ver}"
            removed=true
        fi
    fi
    if [ "$removed" = true ]; then
        build_consolidated_search_file
        log_success "IP $ip removed from blacklist"
        return 0
    else
        log_warning "IP $ip not found in any blacklist"
        return 1
    fi
}
# =============================================================================
...
# NEW DELEGATION FUNCTION: Remove temporary ban
remove_temp_ban() {
    local ip="$1"
    log_info "Removing temporary ban for: $ip"
    local ver
    ver=$(detect_ip_version "$ip")
    if [ "$ver" == "invalid" ]; then
        log_error "Invalid IP address: $ip"
    ...
    local target_set="temp_ban_v${ver}"
    if nft delete element inet "$NFT_TABLE" "$target_set" "{ $ip }" 2>/dev/null; then
        log_success "Removed temporary ban for $ip from $target_set"
        log_ban_attempt "$ip" "MANUAL_UNBAN" "UNBANNED" "Temporary ban manually removed" "N/A" "N/A"
        return 0
    else
        log_warning "IP $ip not found in temporary ban set $target_set"
        return 1
    fi
}
# =============================================================================
# NEW DELEGATION FUNCTIONS: COMPREHENSIVE IP MANAGEMENT
# =============================================================================
# Remove IP from ALL lists (temp bans, permanent blacklist, whitelist, blacklist files)
remove_ip_from_all_lists() {
    local ip="$1"
    log_info "Removing IP from ALL lists: $ip"
    local removed_from=()
    if remove_temp_ban "$ip" 2>/dev/null; then
        removed_from+=("temporary_bans")
    fi
    if remove_from_persistent_blacklist "$ip" 2>/dev/null; then
        removed_from+=("persistent_blacklist")
    fi
    if remove_ip_from_blacklist "$ip" 2>/dev/null; then
        removed_from+=("blacklist_files")
    fi
    if remove_ip_from_whitelist "$ip" 2>/dev/null; then
        removed_from+=("whitelist_files")
    fi
    if [ ${#removed_from[@]} -gt 0 ]; then
        log_success "IP $ip removed from: ${removed_from[*]}"
        return 0
    else
        log_info "IP $ip was not found in any list"
        return 1
    fi
}
# Comprehensive IP check with detailed report
check_ip_comprehensive() {
    local ip="$1"
    echo ""
    echo "==============================================="
    echo "  IP Location Report: $ip"
    echo "==============================================="
    echo ""
    local ver
    ver=$(detect_ip_version "$ip")
    ...
    local geoip_info whois_info
    geoip_info=$(geoip_lookup "$ip")
    whois_info=$(whois_lookup "$ip")
    echo "  GeoIP: $geoip_info"
    echo "  WHOIS: $whois_info"
    echo ""
    echo "Consolidated Search Results:"
    if [ -f "$FAIL2BAN_SEARCH_IPS" ]; then
        local search_results
        search_results=$(search_ip_in_consolidated "$ip" 2>/dev/null || true)
        if [ -n "$search_results" ]; then
            echo "$search_results" | while IFS='|' read -r found_ip source type; do
                if [ "$type" == "WHITELIST" ]; then
                    echo -e "  ${GREEN}✓${NC} Found in: $source ($type)"
                else
                    echo -e "  ${RED}✗${NC} Found in: $source ($type)"
                fi
            done
        else
        ...
        echo "  Consolidated search file not available"
    fi
    echo ""
    echo "nftables Status:"
    if check_nftables_table_exists; then
        local found_in_nft=false
        local sets=("whitelist_v${ver}" "temp_ban_v${ver}" "user_blacklist_v${ver}" "system_blacklist_v${ver}")
        for set in "${sets[@]}"; do
            if nft list set inet "$NFT_TABLE" "$set" 2>/dev/null | grep -q "$ip"; then
                echo -e "  ${YELLOW}✓${NC} Found in nftables set: $set"
                found_in_nft=true
            fi
        done
        if [ "$found_in_nft" = false ]; then
            echo "  Not found in any nftables set"
        fi
    else
        echo "  nftables table not available"
    fi
    echo ""
    echo "Ban History:"
    if [ -f "$BAN_LOG" ]; then
        local total_bans
        total_bans=$(grep -c "|${ip}|" "$BAN_LOG" 2>/dev/null || echo "0")
        if [ "$total_bans" -gt 0 ]; then
            echo "  Total attempts: $total_bans"
            echo ""
            echo "  Recent activity (last 10):"
            grep "|${ip}|" "$BAN_LOG" | tail -10 | while IFS='|' read -r timestamp ip_addr jail action reason geoip whois; do
                local action_color=""
                case "$action" in
                    BANNED) action_color="${RED}" ;;
                    WHITELISTED) action_color="${GREEN}" ;;
                    UNBANNED) action_color="${CYAN}" ;;
                    *) action_color="${NC}" ;;
                esac
                echo -e "    ${action_color}${action}${NC} - $timestamp - Jail: $jail"
            done
        else
            echo "  No ban history found"
        fi
    else
        echo "  Ban log not available"
    fi
    echo ""
    echo "Current Status:"
    if check_ip_whitelisted "$ip"; then
        echo -e "  ${GREEN}✓ WHITELISTED${NC} - This IP will never be banned"
    elif check_ip_in_persistent_blacklist "$ip"; then
        echo -e "  ${MAGENTA}✗ PERMANENTLY BANNED${NC} - Persistent blacklist"
    elif check_ip_in_nftables "$ip"; then
        echo -e "  ${RED}✗ CURRENTLY BANNED${NC} - Active in nftables"
    else
        echo -e "  ${CYAN}➜ NEUTRAL${NC} - Not whitelisted or banned"
    fi
    echo ""
    echo "==============================================="
}
...
# [CONTINUED IN NEXT MESSAGE - SCRIPT IS TOO LONG FOR SINGLE ARTIFACT]
# Copy from original script starting from install_nftban_fail2ban() function
# and all functions after it, ending with main() function
# =============================================================================
# INSTALLATION - Using original from uploaded script
# =============================================================================
### added by copy paste need recheck
# =============================================================================
...
CONSOLIDATED SEARCH:
 --check-cron            Verify cron entry & script integrity
 --force-cron-reinstall  Remove & reinstall the cron job
...
WHITELIST/BLACKLIST OPERATIONS:
    --add-to-whitelist IP [COMMENT]   Add IP to whitelist
    --remove-from-whitelist IP        Remove IP from whitelist
    --check-whitelist IP              Check if IP is whitelisted (returns exit code)
    --remove-from-blacklist IP        Remove IP from blacklist files
    --remove-ban IP                   Remove temporary ban from nftables
    --remove-from-all IP              Remove IP from ALL lists (comprehensive cleanup)
    --verify-ip IP                    Show comprehensive IP status report
TESTING:
...
# Main script
check_cron_job() {
  log_info "Checking cron job for consolidated search updates..."
  local cron_script="$BASE_DIR/scripts/rebuild_consolidated_ips.sh"; local ok=true
  if [ -f "$cron_script" ]; then
    if [ ! -x "$cron_script" ]; then chmod +x "$cron_script" 2>/dev/null || true; fi
    log_info "Cron script present: $cron_script"
  else
    log_error "Cron script missing: $cron_script"; ok=false
  fi
  if crontab -l 2>/dev/null | grep -qF "$cron_script"; then
    log_info "Crontab entry found"
  else
    log_error "Crontab entry not found"; ok=false
  fi
  if [ -f "$FAIL2BAN_SEARCH_IPS" ]; then
    local now=$(date +%s); local ts=$(stat -c %Y "$FAIL2BAN_SEARCH_IPS" 2>/dev/null || echo 0)
    if [ "$ts" -gt 0 ] && [ $(( now - ts )) -le 900 ]; then
      log_success "Consolidated file updated within last 15 minutes"
    else
      log_warning "Consolidated file appears stale"
    fi
  fi
  $ok && { log_success "Cron check OK"; return 0; } || { log_error "Cron check FAILED"; return 1; }
}
force_cron_reinstall() { log_info "Forcing cron reinstall..."; remove_consolidated_file_cron || true; install_consolidated_file_cron || return 1; log_success "Cron reinstalled"; }
remove_cron_job() { log_info "Removing cron job..."; remove_consolidated_file_cron; }
...
main() {
  ...
  --check-cron|--cron-check)
    check_cron_job; exit $? ;;
  --force-cron-reinstall|--cron-reinstall)
    force_cron_reinstall; exit $? ;;
  --remove-cron|--cron-remove)
    remove_cron_job; exit $? ;;
--install)
            install_nftban_fail2ban
            ;;
        --uninstall)
