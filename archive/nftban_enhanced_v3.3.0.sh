...
# Version: 3.3.0 (Enhanced with sync validation and active set checking)
...
show_help() {
  cat << 'EOF'
NFTBan – nftables & Fail2Ban helper (v3.3.0)
===========================================
...
# Auto-sync prompt after perm-ban and remove-ip
auto_sync_prompt() {
    echo -e "${YELLOW}Θέλεις να συγχρονίσεις τις αλλαγές με το nftables τώρα; (y/N)${NC}"
    read -r -n 1 reply
    echo
    if [[ "$reply" =~ ^[Yy]$ ]]; then
        sync_nftables
    else
        echo -e "${CYAN}Μπορείς να τρέξεις 'nftban --sync' αργότερα για συγχρονισμό.${NC}"
    fi
}
...
export_set() {
    local set_name="$1"
    local output_file="$2"
    if [ -z "$set_name" ] || [ -z "$output_file" ]; then
        echo -e "${RED}Χρήση: nftban --export-set <SET_NAME> <OUTPUT_FILE>${NC}"
        return 1
    fi
    if nft list set inet "$TEMP_BAN_TABLE" "$set_name" > "$output_file" 2>/dev/null; then
        echo -e "${GREEN}Το set '$set_name' εξήχθη στο αρχείο: $output_file${NC}"
    else
        echo -e "${RED}Αποτυχία εξαγωγής του set '$set_name'${NC}"
        return 1
    fi
}
...
import_set() {
    ...
    if [ -z "$input_file" ] || [ ! -f "$input_file" ]; then
        echo -e "${RED}Χρήση: nftban --import-set <INPUT_FILE>${NC}"
        return 1
    fi
    if nft -f "$input_file"; then
        echo -e "${GREEN}Το set εισήχθη επιτυχώς από το αρχείο: $input_file${NC}"
    else
        echo -e "${RED}Αποτυχία εισαγωγής του set από το αρχείο${NC}"
        return 1
    fi
}
...
view_log() {
    if [ -f "$LOG_FILE" ]; then
        echo -e "${BLUE}=== Περιεχόμενα αρχείου log: $LOG_FILE ===${NC}"
        cat "$LOG_FILE"
    else
        echo -e "${YELLOW}Το αρχείο log δεν υπάρχει${NC}"
    fi
}
...
clear_log() {
    if [ -f "$LOG_FILE" ]; then
        > "$LOG_FILE"
        echo -e "${GREEN}Το αρχείο log καθαρίστηκε${NC}"
    else
        echo -e "${YELLOW}Το αρχείο log δεν υπάρχει${NC}"
    fi
}
