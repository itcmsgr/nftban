...
**Version (this commit):** v1.109.0 — sourced from [`/VERSION`](VERSION); static per commit, not auto-updated. For the current released tag see [GitHub releases](https://github.com/itcmsgr/nftban/releases) or the README badge.
...
**Schema status:** schema remains frozen at `1.83.0` in v1.109.0. No metric names, labels, or health JSON schema changed across the v1.109 narrow-governance lane. The three v1.100.4 H4 disclosure items (ban attribution under `source=manual`, `EffectiveIdle` for Portscan/LoginMon, shared `input_syn_rate_exceeded` counter) are unchanged. The schema / metrics lane (Lane S) remains held; deeper attribution / effective-state work is routed to a future release (see D-MET-1 / D-METR-2 carried forward). **No metrics, portal, install API, panel-adapter, FHS, sysusers, polkit, dns2-migration-execution, DEGRADED-runtime, or module-isolation changes in v1.109.0.** v1.109.0 is a pure docs / dead-code-removal / dependency-refresh release; zero behavior change.
