...
nftban_cmd_port_help() {
    ...
    echo "  status [options] [ports]          Show port exposure status"
    echo "    Options:"
    echo "      --exposure                    Show service exposure (default)"
    ...
    echo "  Ports are stored in 5 managed nftables port sets per IP family:"
    ...
    echo "    - ssh_ports      SSH listener ports (TCP-only) for the set-driven"
    echo "                     concurrent-connection cap (tcp dport @ssh_ports"
    echo "                     ct count) — PER SOURCE IP (v1.233.0, keyed by"
    echo "                     ip saddr into a named dynamic set), and it"
    echo "                     DROPs over the cap without logging or banning."
    echo "                     INTERNAL/derived: managed automatically"
    echo "                     when nftban detects where sshd listens. Not a"
    echo "                     user-managed service set."
    echo ""
    echo "  SSH listener ports are special (management-plane access). A detected"
    echo "  sshd port goes in BOTH tcp_ports_in (reachability) AND ssh_ports"
    echo "  (concurrent-connection cap), mirrored across IPv4 and IPv6. SFTP, SCP,"
    echo "  rsync-over-SSH and SSH tunnels are covered because they run over sshd."
    echo "  Pure-FTPd, FTP, FTPS, HTTP, mail and DNS are normal services: they use"
    echo "  tcp_ports_in / udp_ports_in only and never ssh_ports. UDP is separate"
    echo "  (udp_ports_in/out); ssh_ports is TCP-only."
    echo ""
    echo "EXAMPLES:"
    echo "  nftban port status               # Show exposure + outbound"
    echo "  nftban port status --exposure    # Show only service exposure"
    ...
    echo ""
    echo "EXPOSURE VIEW (v1.50.1):"
    echo ""
    echo "  Section 1: SERVICE EXPOSURE (Effective Reachability)"
    echo "    Correlates kernel firewall (nft) + runtime sockets (ss)"
    echo "    Columns: PORT, PROTO, SERVICE, PROCESS, FIREWALL, EXPOSURE, DETAIL"
    echo ""
    echo "    EXPOSURE = LISTENING state + FIREWALL state + BIND address"
    echo "    ! EXPOSED   = Listening + firewall allows + bound to network"
    echo "    x BLOCKED   = Not reachable (firewall drops or no rule)"
    echo "    ~ INTERNAL  = Bound to localhost only (never externally reachable)"
    echo "    ? MISCONFIG = Listening but firewall blocks (check intention)"
    echo "    - UNUSED    = Firewall allows but no service running"
    echo "    . CLOSED    = Not running"
    echo ""
    echo "  Section 2: OUTBOUND FIREWALL POLICY"
    echo "    Shows egress ports (dependencies like Zabbix, DNS, SMTP relay)"
    echo ""
    echo "  Section 3: PER-IP ACCESS RULES"
    echo "    Shows per-IP port grants (nftban port allow)"
    echo ""
    echo "SOURCES OF TRUTH:"
    echo "  1. nft list ruleset — kernel firewall state (what is enforced)"
    echo "  2. ss -tulpen       — runtime sockets (what is exposed)"
