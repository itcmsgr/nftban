...
| **socat** | `cmd_cleanup.sh:117,168,248`, `cmd_protect.sh:94`, `cmd_support.sh:1288` | `socat` | daemon UNIX-socket IPC | core-required | `socat` both | YES — lab4 `socat 1.7.4 series (EL9 RPM)`; lab2 `socat` (`/usr/bin/socat1`) | `Depends: socat` | `Requires: socat` | none | `cmd_support.sh:1287` guards; IPC sites do not | PRESENT both | **ALIGNED** |
