...
#
# meta:name="cmd_selftest"
# meta:type="cli"
# meta:header="Self-Test Command (Extended System)"
# meta:version="1.95.0"
# meta:owner="Antonios Voulvoulis <contact@nftban.com>"
# meta:homepage="https://nftban.com"
#
# meta:description="Run deep extended system validation: ban lifecycle, whitelist mutation, integration checks, and detect stuck scripts"
...
[[ -n "${CMD_SELFTEST_LOADED:-}" ]] && return 0
readonly CMD_SELFTEST_LOADED=1
...
nftban_cmd_selftest() {
    ...
    case "$subcommand" in
        ...
        run|test)
            ...
            ;;
        quick)
            nftban_selftest_run --quick "$@"
            ;;
        all|detailed)
            nftban_selftest_run --all "$@"
            ;;
        lifecycle)
            nftban_selftest_run --lifecycle "$@"
            ;;
        verify)
            nftban_selftest_verify "$@"
            ;;
        config|configs)
            nftban_selftest_verify_configs "$@"
            ;;
        check|orphans)
            nftban_selftest_check_orphans "$@"
            ;;
        stats)
            nftban_selftest_stats
            ;;
        trace)
            nftban_selftest_trace "$@"
            ;;
        help|--help|-h)
            nftban_cmd_selftest_usage
            ;;
        *)
...
# =============================================================================
# SMART VERIFICATION (nftban selftest verify)
# =============================================================================
...
# =============================================================================
# CONFIG INTEGRITY VERIFICATION (nftban selftest config)
# =============================================================================
...
nftban_cmd_selftest_usage() {
    cat <<EOF
Usage: nftban selftest <command> [OPTIONS]

Deep extended system validation suite. NOT for routine CI smoke..
...
Examples:
  nftban selftest run              # Standard smoke test (~32 commands)
  nftban selftest quick            # Quick test (3 commands)
  nftban selftest all              # Test ALL CLI commands + lifecycle
  nftban selftest lifecycle        # Ban lifecycle tests only
  nftban selftest verify           # Run all smart verification tests
  nftban selftest verify bans      # Verify ban count changes only
  nftban selftest verify feeds     # Verify feeds loaded correctly
  nftban selftest verify geoban    # Verify country blocks applied
  nftban selftest config           # Verify runtime config files exist
  nftban selftest config build     # CI: verify source files exist (use with NFTBAN_PROJECT_ROOT)
  nftban selftest check            # Check for stuck scripts
  nftban selftest check 10         # Check traces older than 10 min
  nftban selftest stats            # Trace statistics
  nftban selftest trace recent 50  # Last 50 trace entries
...
Config Integrity Check:
  ...
  CI Usage:
    NFTBAN_PROJECT_ROOT=/path/to/nftban nftban selftest config build
...
# Export functions
export -f nftban_cmd_selftest
export -f nftban_cmd_selftest_usage
