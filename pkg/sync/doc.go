...
// # Overview
//
// The sync package handles the core operation of comparing desired state
...
//	merged := sync.MergeCIDRs(cidrs)
//	// Result: ["10.0.0.0/23"] (two /24s merged)
//
// # Sync Flow
//
//	// 1. Get current state from nftables
//	current := sync.GetSetElements("ip", "nftban", "blacklist_ipv4")
//
//	// 2. Compute diff
//	toAdd, toRemove := sync.Diff(desired, current)
//
//	// 3. Apply changes atomically
//	sync.Apply(toAdd, toRemove)
//
// # Thread Safety
//
