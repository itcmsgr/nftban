...
declare -A PRIOR_CLASS=() PRIOR_MODE=() PRIOR_REPRO=() PRIOR_NOTE=() PRIOR_OCC=() PRIOR_CONS=()
if [[ -f "$OUT_REG" ]]; then
    ...
    while IFS= read -r _line; do
        ...
        # occurrences and consumers are carried too: a row moved to the ledger
        # must be LOSSLESS, and "every field" includes the two that describe what
        # was actually there.
        PRIOR_OCC["$_key"]="${_f[3]:-1}"
        PRIOR_CONS["$_key"]="${_f[4]:-}"
        if [[ "${#_f[@]}" -ge 9 ]]; then
...
HDR
    # LC_ALL=C — see the ledger's sort below: byte order, not locale order, so a
    # no-op regeneration is byte-identical on every machine.
...
for _key in "${!PRIOR_CLASS[@]}"; do
    ...
    # LOSSLESS MOVE. Every registry field is carried, including occurrences and
    # consumers. reproduction_status is copied VERBATIM and is never upgraded:
    # "the shape was eliminated" and "the defect was reproduced" are separate
    # facts, and a row that was UNVERIFIED when it disappeared is UNVERIFIED in
    # the record. Promoting it because someone happened to delete the line would
    # manufacture evidence of a reproduction that never happened.
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$_kp" "$_kf" "$_kfp" \
        "${PRIOR_OCC[$_key]:-1}" "${PRIOR_CONS[$_key]:--}" \
        "${PRIOR_CLASS[$_key]:-UNCLASSIFIED}" \
...
#
# REMEDIATED LEDGER — declared exposures that are NO LONGER DETECTED, with every
# field they carried when last seen.
#
# ⛔ AUTHORITATIVE STATE MACHINE — this is the definition, not a summary of one.
# It is written here so it cannot be reinterpreted later from behaviour.
#
#     ACTIVE REGISTRY        currently detectable exposure
#        |  subject shape disappears AND source file survives
#        v
#     REMEDIATED LEDGER      historical record of DISAPPEARANCE
#                            NOT proof the defect was reproduced or fixed
#        |  same fingerprint returns
#        v
#     FAIL                   PIPEFAIL_EPIPE_REMEDIATION_REGRESSED
#
#     source file itself disappears -> ORPHAN -> human review, never REMEDIATED
#
# A fingerprint is in EXACTLY ONE state. Present in both the active registry and
# this ledger is a contradiction, not a redundancy, and the gate fails it as
# EPIPE_LEDGER_DOUBLE_STATE.
#
# ⛔ reproduction_status IS COPIED VERBATIM AND NEVER UPGRADED. "The shape was
# eliminated" and "the defect was reproduced" are separate facts. A row that was
# UNVERIFIED when it disappeared stays UNVERIFIED here; promoting it because
# someone deleted the line would manufacture evidence of a reproduction that
# never happened. Likewise a PROVEN row stays PROVEN: the reproduction happened
# and the record of it does not expire when the line does.
#
...
#
# plane	file	fingerprint	occurrences	consumers	last_class	last_mode	last_reproduction_status	note
HDR
    # LC_ALL=C: byte order, not locale order. Collation differs between locales,
    # so an unset LC_ALL makes the same tree produce different bytes on different
    # machines — and a generator whose output drifts on a no-op run makes every
    # future STALE/ORPHAN signal untrustworthy, because a real product change can
    # no longer be told from generator noise.
    LC_ALL=C sort < "$LEDGER_TMP"
} > "$OUT_LEDGER"
