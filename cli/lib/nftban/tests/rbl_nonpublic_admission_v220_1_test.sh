...
run_cmd() { # $1 = shell snippet run after stubs; prints output + "CHECKED:<ip>" lines
  ...
  (
    ...
    nftban_rbl_get_public_ips() { printf '%s\n' "198.51.100.67" "2001:db8:c014:5ee1::1"; }
...
o="$(run_cmd 'NFTBAN_RBL_CRITICAL_IPS="198.51.100.67|dup" nftban_cmd_rbl_check --quiet 2>&1')"
n="$(printf '%s\n' "$o" | grep -c '^CHECKED:198.51.100.67$' || true)"
